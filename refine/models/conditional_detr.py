...
class ConditionalDETR_Refine(nn.Module):
    ...
    def forward(self, samples: NestedTensor, return_memory=False):
        ...
        self.transformer.H = H // self.backbone[0].body.patch_size
        self.transformer.W = W // self.backbone[0].body.patch_size
        features, pos = self.backbone(samples)
        src, mask = features['x_patch'].decompose()
        assert mask is not None
        Hs, references, memory = self.transformer(src, mask, self.query_embed.weight, pos[-1],
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, focal_alpha, losses, gamma, box_jitter, args):
        ...
        self.extend_rel_token = args.extend_rel_token
        self.pair_attn_cls = args.pair_attn_cls
        self.add_relation_token = args.add_relation_token
        self.num_attn_classes = 3
        self.num_spatial_classes = 6
        self.num_contacting_classes = 17
        self.num_object_classes = 36
        self.num_relation_classes = self.num_attn_classes + self.num_spatial_classes + self.num_contacting_classes
    ...
    def loss_cams(self, outputs, targets, *args, **kwargs):
        if 'cams_cls' not in outputs:
            return {'loss_cams': torch.tensor(0.0).to(outputs['pred_logits'])}
        ...
        for i, target in enumerate(targets):
            ...
            if not target['is_full']:
                continue
            cams = outputs['cams_cls'][i]
            for cid in range(1, self.num_object_classes + 1):
                boxes = box_ops.box_cxcywh_to_xyxy(target['ori_boxes'])
                labels = target['ori_labels']
                idxes = labels == cid
                if idxes.any():
                    cam = cams[cid - 1]
                    h, w = cam.shape
                    cam_scale = torch.tensor([w, h, w, h]).to(boxes)
                    tgt_cam = torch.zeros_like(cam)
                    sel_boxes = boxes[idxes]
                    for box in sel_boxes:
                        x0, y0, x1, y1 = (box * cam_scale).long()
                        tgt_cam[y0:y1+1, x0:x1+1] = 1
                    lb_loss = F.binary_cross_entropy(cam.view(-1), tgt_cam.view(-1))
                    cur_loss_list.append(lb_loss)
            loss_list.append(torch.mean(torch.tensor(cur_loss_list)))
        loss_cams = torch.mean(torch.tensor(loss_list)) if len(loss_list) else torch.tensor(0.0).to(outputs['pred_logits'])
        losses = {'loss_cams': loss_cams}
        return losses
    ...
    def loss_img_label(self, outputs, targets, *args, **kwargs):
        ...
        target_class = torch.stack([t["img_label"] for t in targets]).to(logits).float()
        loss_label_tokens = F.binary_cross_entropy_with_logits(tokens_logits, target_class)
        if self.extend_rel_token:
            target_class = torch.zeros(len(targets), self.num_object_classes, self.num_relation_classes).to(logits)
            for batch_i in range(len(targets)):
                img_label = torch.where(targets[batch_i]['img_label'])[0]
                target_class[batch_i, img_label, :self.num_attn_classes] = targets[batch_i]['attention_label'].float()
                target_class[batch_i, img_label, self.num_attn_classes:-self.num_contacting_classes] = targets[batch_i]['spatial_label'].float()
                target_class[batch_i, img_label, -self.num_contacting_classes:] = targets[batch_i]['contact_label'].float()
            logits = logits.view(len(targets), self.num_object_classes, -1)

            # matched loss
            # row_valid_index = torch.where(target_class.sum(dim=-1))
            # target_class = target_class[row_valid_index]
            # logits = logits[row_valid_index]
        ...
        if self.pair_attn_cls:
            target_one_hot_class = target_class.long()
            # attn_logits = outputs['x_attn_logits'][:, 1:][target_one_hot_class[:, 1:] == 1]
            # spat_logits = outputs['x_spat_logits'][:, 1:][target_one_hot_class[:, 1:] == 1]
            # cont_logits = outputs['x_cont_logits'][:, 1:][target_one_hot_class[:, 1:] == 1]

            # target_attn_label = torch.cat([t['attention_label'][1:] for t in targets]).to(logits.get_device()).argmax(-1)
            # target_spat_label = torch.cat([t['spatial_label'][1:] for t in targets]).to(logits.get_device()).float()
            # target_cont_label = torch.cat([t['contact_label'][1:] for t in targets]).to(logits.get_device()).float()

            # loss_attn = F.cross_entropy(attn_logits, target_attn_label)
            # loss_spat = F.binary_cross_entropy_with_logits(spat_logits, target_spat_label)
            # loss_cont = F.binary_cross_entropy_with_logits(cont_logits, target_cont_label)

            # add negative supervision
            target_attn_label = torch.zeros_like(outputs['x_attn_logits'])
            target_spat_label = torch.zeros_like(outputs['x_spat_logits'])
            target_cont_label = torch.zeros_like(outputs['x_cont_logits'])
            idxes = torch.where(target_one_hot_class)

            attn_logits = outputs['x_attn_logits']
            spat_logits = outputs['x_spat_logits']
            cont_logits = outputs['x_cont_logits']

            target_attn_label_o = torch.cat([t['attention_label'] for t in targets]).to(logits.get_device()).float()
            target_spat_label_o = torch.cat([t['spatial_label'] for t in targets]).to(logits.get_device()).float()
            target_cont_label_o = torch.cat([t['contact_label'] for t in targets]).to(logits.get_device()).float()
            
            target_attn_label[idxes] = target_attn_label_o
            target_spat_label[idxes] = target_spat_label_o
            target_cont_label[idxes] = target_cont_label_o

            loss_attn = F.binary_cross_entropy_with_logits(attn_logits[:, 1:], target_attn_label[:, 1:])
            loss_spat = F.binary_cross_entropy_with_logits(spat_logits[:, 1:], target_spat_label[:, 1:])
            loss_cont = F.binary_cross_entropy_with_logits(cont_logits[:, 1:], target_cont_label[:, 1:])

            losses.update({"loss_attn_cls": loss_attn, "loss_spat_cls": loss_spat, "loss_cont_cls": loss_cont})

        if self.add_relation_token:
            target_attn_label = torch.stack([t['attention_label'].any(dim=0) for t in targets]).float()
            target_spat_label = torch.stack([t['spatial_label'].any(dim=0) for t in targets]).float()
            target_cont_label = torch.stack([t['contact_label'].any(dim=0) for t in targets]).float()

            attn_logits = outputs['x_attn_logits']
            spat_logits = outputs['x_spat_logits']
            cont_logits = outputs['x_cont_logits']
            
            loss_attn = F.binary_cross_entropy_with_logits(attn_logits, target_attn_label)
            loss_spat = F.binary_cross_entropy_with_logits(spat_logits, target_spat_label)
            loss_cont = F.binary_cross_entropy_with_logits(cont_logits, target_cont_label)
    ...
    def forward(self, outputs, targets):
        ...
        if self.training:
            for i in range(len(targets_cp)):
                ...
                targets_cp[i]['ori_boxes'] = targets[i]['boxes']
                targets_cp[i]['ori_labels'] = targets[i]['labels']
...
class PostProcess(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, target_sizes, keep_queries=100, cls_dict=None, need_scale=True):
        ...
        if cls_dict:
            img_prob = torch.cat((torch.zeros_like(cls_dict['x_logits'][:, :1]), cls_dict['x_logits'].sigmoid()), dim=1).unsqueeze(1)
            attn_prob = torch.cat((torch.zeros_like(cls_dict['x_cls_logits'][:, :1]), cls_dict['x_cls_logits'].sigmoid()), dim=1).unsqueeze(1)
            cls_weight = (img_prob + attn_prob) / 2
            # cls_weight = attn_prob
            prob = prob * cls_weight

        topk_values, topk_indexes = torch.topk(prob.view(out_logits.shape[0], -1), keep_queries, dim=1)
        if cls_dict:
            scores = torch.gather(out_logits.sigmoid().view(out_logits.shape[0], -1), 1, topk_indexes)
        else:
            scores = topk_values
...
# class InferPostProcess(nn.Module):
#     """ This module converts the model's output into the format expected by the coco api"""

#     @torch.no_grad()
#     def forward(self, outputs, target_sizes, keep_queries=100, threhsold=0.2, need_scale=True):
#         """ Perform the computation
#         Parameters:
#             outputs: raw outputs of the model
#             target_sizes: tensor of dimension [batch_size x 2] containing the size of each images of the batch
...
#         img_h, img_w = target_sizes.unbind(1)
#         scale_fct = torch.stack([img_w, img_h, img_w, img_h], dim=1)
#         out_bbox = box_ops.box_cxcywh_to_xyxy(out_bbox).clamp(min=0, max=1)
#         if need_scale:
#             out_bbox = out_bbox * scale_fct[:, None, :]

#         # get the predictions whose score are above the threshold
#         scores, labels, boxes = [], [], []
#         for batch_i in range(len(prob)):
#             keep_idx = (prob[batch_i] >= threhsold).nonzero()
#             if keep_idx.shape[0] < 10:
#                 _, topk_indexes = torch.topk(prob[batch_i].flatten(), k=10)
#                 keep_idx = torch.ones(10, 2, dtype=torch.long, device=prob.device)
#                 keep_idx[:, 0] = topk_indexes // out_logits.shape[2]
#                 keep_idx[:, 1] = topk_indexes % out_logits.shape[2]

#             boxes.append(out_bbox[batch_i, keep_idx[:, 0]])
#             labels.append(keep_idx[:, 1])
#             scores.append(prob[batch_i, keep_idx[:, 0], keep_idx[:, 1]])
#         results = [{'scores': s, 'labels': l, 'boxes': b} for s, l, b in zip(scores, labels, boxes)]
#         return results
...
class InferPostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes, keep_queries=100, threhsold=0.2, need_scale=True):
        ...
        prob = out_logits.sigmoid()
        img_h, img_w = target_sizes.unbind(1)
        boxes = box_ops.box_cxcywh_to_xyxy(out_bbox).clamp(min=0, max=1)
        if need_scale:
            scale_fct = torch.stack([img_w, img_h, img_w, img_h], dim=1)
            boxes = boxes * scale_fct[:, None, :]
        ...
        for ii in range(len(targets)):
            ...
            for cc in range(out_logits.shape[2]):
                if cc in targets[ii]['labels']:
                    ...
                    tmp_boxes.append(top_boxes[ii][cc].reshape(1, -1)) # TODO: only one box is obtained here ?
...
def build(args):
    ...
    num_classes = 21 if args.dataset_file != 'coco' else 36 + 1 # 91
    ...
    weight_dict = {'loss_ce': args.cls_loss_coef,
                   ...
                   'loss_attn_cls': args.rel_loss_coeff, 
                   'loss_spat_cls': args.rel_loss_coeff,
                   'loss_cont_cls': args.rel_loss_coeff,
                   'loss_cams': args.img_label_loss_coef }
    ...
    losses = ['cams', 'labels', 'boxes', 'cardinality', 'image_label']
    losses_refine = ['labels', 'boxes', 'cardinality']

    if args.masks:
        losses += ["masks"]
    criterion = SetCriterion(num_classes, matcher=matcher, weight_dict=weight_dict,
                             focal_alpha=args.focal_alpha, losses=losses, gamma=args.focal_gamma, box_jitter=args.box_jitter, args=args)
    criterion.to(device)
    postprocessors = {'bbox': PostProcess()} if not args.infer_save else {'bbox': InferPostProcess()}
    ...
    criterion_refine = SetCriterionRefine(num_classes, matcher=matcher_refine, weight_dict=weight_dict,
                                          focal_alpha=args.focal_alpha, losses=losses_refine, gamma=args.focal_gamma, box_jitter=args.box_jitter, args=args)

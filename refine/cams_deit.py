...
def draw_gt_bbox(image, gt_box_list, pred_box_list, cls_name):

    def draw_bbox(img, gt_box_list, box_list, color1=(0, 0, 255), color2=(0, 255, 0)):
        ...
        for i in range(len(box_list)):
            ...
            #print(box_list[i,0], box_list[i,1], box_list[i,2], box_list[i,3])
            cv2.rectangle(img, (box_list[i][0], box_list[i][1]), (box_list[i][2], box_list[i][3]), color1, 2)
            cv2.putText(img, cls_name, (box_list[i][0], box_list[i][1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    ...
    # draw bbox on image
    boxed_image = draw_bbox(boxed_image, gt_box_list, pred_box_list)

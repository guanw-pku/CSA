...
from lib.assign_pseudo_label import load_feature, assign_label_to_proposals_by_dict_for_video, convert_data, category_oi2ag, create_dis_list, assign_label_to_general_detector
...
import torchvision
from lib.extract_bbox_features import extract_feature_given_bbox
import pdb
...
class detector(nn.Module):
    ...
    def wk_forward(self, im_data, im_info, gt_boxes, num_boxes, gt_annotation, frame_names, faset_rcnn_model, transforms):
        dets_list, feat_list, base_feat_list = load_feature(frame_names, self.union_box_feature, load_feat=True)

        if feat_list == None:
            feat_list = self.extract_feat(dets_list, frame_names, faset_rcnn_model, transforms)
    ...
    def extract_feat(self, det_list, frame_names, faset_rcnn_model, transforms):
        ...
        for i, det in enumerate(det_list):
            feats = []
            boxes = torch.tensor([det[i]['rect'] for i in range(len(det))])
            if len(boxes) > 0:
                boxes_feat = extract_feature_given_bbox(faset_rcnn_model, transforms, cv2_imgs[i], boxes).mean([2,3])
                feats.append(boxes_feat)
                ...
                feats.append(torch.Tensor([]).cuda(0))
            if len(feats) > 0:
    ...
    def load_oracle_detection(self, gt_annotation):
        dets_list = []
        for gt in gt_annotation:
            frame_det = []
            for obj in gt:
                obj_dct = {}
                if 'person_bbox' in obj:
                    obj_dct['class'] = 1
                    obj_dct['rect'] = obj['person_bbox'][0].tolist()
                else:
                    obj_dct['class'] = obj['class']
                    obj_dct['rect'] = list(obj['bbox'])
                obj_dct['conf'] = 1.0
                frame_det.append(obj_dct)
            dets_list.append(frame_det)

...
def prepare_func():
    ...
    opts = ["MODEL.WEIGHT", "/home/wangguan/PLA/models/vinvl/vinvl_vg_x152c4.pth", 
...
if __name__ == "__main__":
    ...
    data_root = '/home/wangguan/PLA/data/action-genome/frames'
    save_root = '/home/wangguan/PLA/data/action-genome/AG_detection_results'
    for vid in tqdm(os.listdir(data_root)[9000:]):
        vid_folder = os.path.join(data_root, vid)
        save_vid_path = os.path.join(save_root, vid)
        
        for frame in os.listdir(vid_folder):
            save_frame_path = os.path.join(save_vid_path, frame)
            if not os.path.exists(save_frame_path):
                os.makedirs(save_frame_path)
            # save_base_feat_path = os.path.join(save_frame_path, 'base_feat.npy')
            save_det_path = os.path.join(save_frame_path, 'dets.npy')
            save_roi_path = os.path.join(save_frame_path, 'feat.npy')
            # if os.path.exists(save_base_feat_path) and os.path.exists(save_det_path) and os.path.exists(save_roi_path):
            #     continue

            # if os.path.exists(save_det_path) and os.path.exists(save_roi_path):
            #     continue

            frame_path = os.path.join(vid_folder, frame)
            cv2_img = cv2.imread(frame_path)

            # # obtain image base feature 
            # img_base_feat = extract_base_feature_one_img(model, transforms, cv2_img)[0]
            # img_base_feat = img_base_feat.cpu().numpy()
            # pdb.set_trace()
            # if not os.path.exists(save_base_feat_path):
            #     np.save(save_base_feat_path, img_base_feat)
            ...
            ori_width = cv2_img.shape[1]
            ori_wh = (ori_width,ori_height)
            orig_pred = prediction.resize(ori_wh)
            
            rect = orig_pred.bbox
            conf = orig_pred.extra_fields['scores']
            label = orig_pred.extra_fields['labels']
            pdb.set_trace()

            det_list = []
            for idx in range(len(rect)):
                det_dict = {'rect': rect[idx], 'class': label[idx], 'conf': conf[idx]}
                det_list.append(det_dict)
            
            # if not os.path.exists(save_det_path):
                # np.save(save_det_path, det_list)
            
            # obtain roi feature
            roi_feat = extract_feature_given_bbox(model, transforms, cv2_img, rect)
            roi_feat = roi_feat.mean([2,3]).cpu().numpy()
            # if not os.path.exists(save_roi_path):
            #     np.save(save_roi_path, roi_feat)

...
def load_feature(frame_names, union_box_feature, path='/home/wangguan/CSA/data/action-genome/AG_detection_results_refine', load_feat=True):
    """
...
def assign_label_to_proposals_by_dict_for_video(dets, feats, is_train, gt_annotation, dict_path='data/action-genome/annotations/weak', pseudo_way=0):
...
def assign_label_to_general_detector(dets_list):
    oi_to_ag_cls_dict = np.load(os.path.join('data/action-genome/annotations/weak', 'oi_to_ag_word_map_synset.npy'), allow_pickle=True).tolist()
    final_list = []
...
def convert_data(is_train, base_feat_list, video_people_det, video_people_feat, video_object_det, video_object_feat, \
    ...
    features = torch.zeros((bbox_num, 192), device=MyDevice) # TODO: ours 192, baseline or roi feature 2048
    im_idx = []
    pair_idx = []
    a_rel = []
    s_rel = []

...
class BasicSceneGraphEvaluator:
    ...
    def evaluate_scene_graph(self, gt, pred):
        ...
        for idx, frame_gt in enumerate(gt):
            ...
            # gt_entry
            # gt_classes: 一维列表，obj_num大小，每个obj的class_id
            # gt_relations: 二维列表，rel_num*3大小，[obj1_id, obj2_id, rel_id]
            # gt_boxes: 二维列表，obj_num*4大小，[x1, y1, x2, y2]
            gt_entry = {
            ...
            if pred == {}:
                for k in self.result_dict[self.mode + '_recall']:
                    ...
                    rec_i = float(0) / float(gt_entry['gt_relations'].shape[0])
            ...
            if self.mode == 'predcls':
                ...
                # pred_entry
                # pred_boxes: 二维列表，obj_num*4大小，[x1, y1, x2, y2]
                # pred_classes: 一维列表，obj_num大小，每个obj的class_id
                # pred_rel_inds: 二维列表，rel_num*2大小，[obj1_id, obj2_id]
                # obj_scores: 一维列表，obj_num大小，每个obj的score
                # rel_scores: 一维列表，rel_num大小，每个rel的score
                pred_entry = {
                    'pred_boxes': pred['boxes'][:, 1:].cpu().clone().numpy(),
...
def evaluate_from_dict(gt_entry, pred_entry, mode, result_dict, method=None, threshold = 0.9, save_res=False, **kwargs):
    """
    ...
    if method == 'semi':
        ...
        score_inds = argsort_desc(overall_scores)[:100] # TODO 100
    ...
    # After with/semi/no constraint
    # pred_rels: 二维列表，rel_num*3大小，[obj1_id, obj2_id, rel_id]
    # predicate_scores: 一维列表，rel_num大小，每个rel的score

    # pred_to_gt: 一维列表，rel_num大小，列表中每个元素还是一个大小不定的列表，Matching from predicate to GT
    # pred_5ples: 二维列表，rel_num*5大小，the predicted (id0, id1, cls0, cls1, rel)
    # rel_scores: 二维列表，rel_num*3大小，[cls_0score, cls1_score, relscore]
...
def _compute_pred_matches(gt_triplets, pred_triplets,
    ...
    keeps = intersect_2d(gt_triplets, pred_triplets) # TODO

...
class AG(Dataset):
    ...
    filter_nonperson_box_frame=True, filter_small_box=False, need_relation=True, output_log=False):
        ...
        if ws_object_bbox_path is None:
            ...
            with open(os.path.join(root_path, 'annotations/weak/gt_annotation_thres02.pkl'), 'rb') as f:
        ...
        if datasize == 'mini':
            ...
            for i in list(person_bbox.keys())[:1000]:
                ...
                real_small_person[i] = real_object_bbox[i]
                real_small_object[i] = real_person_bbox[i]
            person_bbox = small_person
            object_bbox = small_object
            real_person_bbox = real_small_person
            real_object_bbox = real_small_object
        ...
        for i in video_dict.keys():
            ...
            for j in video_dict[i]:
                if filter_nonperson_box_frame:
                    if person_bbox[j]['bbox'].shape[0] == 0:
                        ...
                        video.append(j)
                        self.valid_nums += 1
                gt_annotation_frame = self.get_frame_anno(person_bbox[j], object_bbox[j], real=False)
                real_gt_annotation_frame = self.get_frame_anno(real_person_bbox[j], real_object_bbox[j], real=True)

                gt_annotation_video.append(gt_annotation_frame)
                real_gt_annotation_video.append(real_gt_annotation_frame)
            ...
            if len(video) > 2:
                ...
                self.real_gt_annotations.append(real_gt_annotation_video)
            # 添加的部分
        ...
        # ######################    
        # video_list_concat = []
        # for lst in self.video_list:
        #     video_list_concat += lst
        
        # total_train_coco_file = '/home/wangguan/PLA/data/action-genome/annotations/ag_train_coco_style_with_one_frame.json'
        # save_sel_train_coco_path = '/home/wangguan/PLA/data/action-genome/annotations/weak/weak_ag_det_coco_style.json'
        # with open(total_train_coco_file, 'r') as f:
        #     total_train_coco = json.load(f)
        # sel_train_coco = {'images': [], 'annotations': [], 'categories': [], 'rel_categories': []}
        # sel_train_coco['categories'] = total_train_coco['categories']
        # sel_train_coco['rel_categories'] = total_train_coco['rel_categories']

        # sel_fname_list = []
        # for fname in video_list_concat:
        #     sel_fname_list.append('frames/' + fname)
        # cur_img_idx = 1
        # cur_anno_idx = 1
        # cur_anno_iter = 0
        # total_train_coco_imgs = total_train_coco['images']
        # total_train_coco_annos = total_train_coco['annotations']
        # for img_dict in tqdm(total_train_coco_imgs):
        #     fname = img_dict['file_name']
        #     if fname in sel_fname_list:
        #         sel_frame_dict = {}
        #         sel_frame_dict.update(img_dict)
        #         sel_frame_dict['id'] = cur_img_idx
        #         del sel_frame_dict['video_id']
        #         sel_train_coco['images'].append(sel_frame_dict)
        #         flag = False
        #         while cur_anno_iter < len(total_train_coco_annos):
        #             if total_train_coco_annos[cur_anno_iter]['image_id'] < img_dict['id']:
        #                 cur_anno_iter += 1
        #                 continue
        #             if total_train_coco_annos[cur_anno_iter]['image_id'] > img_dict['id']:
        #                 break
        ...
        #             anno_dict = {}
        #             anno_dict.update(total_train_coco_annos[cur_anno_iter])
        #             anno_dict['id'] = cur_anno_idx
        #             anno_dict['image_id'] = cur_img_idx
        #             del anno_dict['video_id']
        #             sel_train_coco['annotations'].append(anno_dict)
        #             cur_anno_idx += 1
        #             cur_anno_iter += 1
        #             flag=True
        #         if not flag:
        ...
        #         cur_img_idx += 1
        # with open(save_sel_train_coco_path, 'w') as f:
        #     json.dump(sel_train_coco, f)
        # pdb.set_trace()
        # print('hello')
    ...
    def get_frame_anno(self, person_bbox, object_bbox, real=False):
        gt_annotation_frame = [{'person_bbox': person_bbox['bbox']}]
        # each frames's objects and human
        for k in object_bbox:
            if k['visible']:
            # if k['bbox'] != None and k['visible']:
                assert k['bbox'] != None, 'warning! The object is visible without bbox'
                # 只保留三类关系都有的object
                if self.need_relation == False:
                    if not real:
                ...
                elif self.need_relation and k['attention_relationship'] is not None and k['spatial_relationship'] is not None and k['contacting_relationship'] is not None and \
